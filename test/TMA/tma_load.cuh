...
__global__ void tma_load_kernel(__grid_constant__ const TmaLoad tma_load, GmemTensor gmem_tensor, SmemLayout smem_layout, T* d_out, int ld) {
    ...
    if (threadIdx.x == 0) {
        int row_base = blockIdx.x * CTA_M;
        int col_base = blockIdx.y * CTA_N;
        for (int m = 0; m < CTA_M; ++m) {
            for (int n = 0; n < CTA_N; ++n) {
                d_out[(row_base + m) * ld + (col_base + n)] = smem_data[m * CTA_N + n];
            }
        }
    }
}
...
void tma_load(T* data, T* d_out, int M, int N) {
    using namespace cute;
    ...
    tma_load_kernel<T, CTA_M, CTA_N, decltype(tma_load), decltype(gmem_tensor), decltype(smem_layout)>
                   ...
                   (tma_load, gmem_tensor, smem_layout, d_out, N);
